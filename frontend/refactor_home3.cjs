...
rightBlock = rightBlock.replace('<div className="space-y-8">\n          <WeatherSportsWidget />', '<WeatherSportsWidget />'); // remove extra wrapper if possible, actually let's just leave it, it's fine. Wait, better to remove the wrapper.
rightBlock = rightBlock.replace('<div className="space-y-8">\n', '');
rightBlock = rightBlock.replace('          {/* Trending Vertical List */}', '        {/* Trending Vertical List */}');
rightBlock = rightBlock.replace('        </div>\n      </div>', '      </div>'); // remove one closing div
